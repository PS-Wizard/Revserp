...
func TestRegistry(t *testing.T) {
	...
	if names := registry.Names(); !slices.Equal(names, []string{"read_issues", "get_score_summary", "get_search_console_data", "get_business_profile", "read_issue_work", "read_page", "render_chart", "update_business_profile"}) {
		t.Fatalf("Names() = %v, want the eight served tools", names)
	}
	defs := registry.Defs()
	if len(defs) != 8 {
		t.Fatalf("Defs() = %d defs, want 8", len(defs))
	}
