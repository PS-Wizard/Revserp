...
func TestRegistry(t *testing.T) {
	...
	if names := registry.Names(); !slices.Equal(names, []string{"read_issues", "get_score_summary", "get_search_console_data", "get_business_profile"}) {
		t.Fatalf("Names() = %v, want the three served tools", names)
	}
	defs := registry.Defs()
	if len(defs) != 4 {
		t.Fatalf("Defs() = %d defs, want 4", len(defs))
	}
