...
func readIssuesTool() Tool {
	return Tool{
		Def: Def{
			Name:        "read_issues",
			Label:       "Read issues",
			Description: "Read crawl issues of the current crawl with optional filters and paging. It is the evidence for what issues exist, how many, where, and how to fix them. Returns matching totals, a breakdown of top buckets and issue types with counts and severities, issue rows, next_offset, and has_more. Each row carries url, pillar, bucket, issue_type, severity, message, details, and a deterministic recommended_fix; adapt the fix to the specific row and never invent counts or fixes. Filter by pillars, bucket, issue_type, severity, and urls; limit sets the page size up to 50 (default 25), offset advances the cursor, and the next page is fetched by passing next_offset back as offset. You can filter by one or several pillars at once (for example [\"seo\",\"aeo\"]): with several pillars the rows come back as one interleaved stream — one row from each pillar in turn, ordered by severity within each pillar, every row carrying its pillar — so a seo+aeo+pagespeed request returns a fair mix, never one pillar only. The limit applies to that combined stream, NOT per pillar, and is capped at 30 total rows when several pillars are requested: requesting limit 50 with seo+aeo+pagespeed returns 30 mixed rows, while total_matching and the breakdown still show the full matching set. The breakdown lists valid bucket and issue_type ids; use those exact ids in later filters instead of guessing them. Use one pillar alone when you want to page deep with offset.",
