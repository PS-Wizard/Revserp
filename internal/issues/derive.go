...
func DeriveIssues(pageFacts []shared.PageFact, linkFacts []shared.LinkFact) []shared.DerivedIssue {
	...
	seoFacts := slices.Clone(scoreablePageFacts)
	seo.EnrichPageFactsWithContentFingerprints(seoFacts)
	derivedIssues = append(derivedIssues, seo.DeriveIssues(seoFacts, linkFacts)...)
	derivedIssues = append(derivedIssues, aeo.DeriveIssues(scoreablePageFacts, linkFacts, len(scoreablePageFacts))...)
