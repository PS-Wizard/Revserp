...
func TestAPIKeyTenantIsolationIntegration(t *testing.T) {
	...
	t.Run("user A cannot read user B organization resources", func(t *testing.T) {
		cases := []struct {
			...
			{"crawl pages list", "/v1/crawls/" + b.crawlID.String() + "/pages", http.StatusNotFound},
			{"crawl issues list", "/v1/crawls/" + b.crawlID.String() + "/issues", http.StatusNotFound},
			{"crawl links list", "/v1/crawls/" + b.crawlID.String() + "/links", http.StatusNotFound},
