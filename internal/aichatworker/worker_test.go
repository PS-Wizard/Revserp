...
func TestAllowedTools(t *testing.T) {
	...
	if len(all) != 5 {
		t.Fatalf("allowedTools(nil) has %d tools, want 5: %+v", len(all), all)
	}
	...
	for _, name := range []string{"read_issues", "get_score_summary", "get_search_console_data", "get_business_profile", "read_issue_work"} {
		if !names[name] {
			t.Fatalf("allowedTools(nil) missing %s: %+v", name, all)
		}
	}
	if got := allowedTools([]string{"read_issues"}); len(got) != 4 {
		t.Fatalf("allowedTools(disabled read_issues) = %+v, want the other four", got)
	}
	if got := allowedTools([]string{"read_issues", "get_score_summary", "get_search_console_data", "get_business_profile", "read_issue_work"}); len(got) != 0 {
		t.Fatalf("allowedTools(all disabled) = %+v, want none", got)
	}
	if got := allowedTools([]string{"unknown", "read_issues"}); len(got) != 4 {
		t.Fatalf("allowedTools(unknown+disabled) = %+v, want the other four", got)
	}
}
