...
func TestAllowedTools(t *testing.T) {
	...
	if len(all) != 3 {
		t.Fatalf("allowedTools(nil) has %d tools, want 3: %+v", len(all), all)
	}
	...
	for _, name := range []string{"read_issues", "get_score_summary", "get_search_console_data"} {
		if !names[name] {
			t.Fatalf("allowedTools(nil) missing %s: %+v", name, all)
		}
	}
	if got := allowedTools([]string{"read_issues"}); len(got) != 2 {
		t.Fatalf("allowedTools(disabled read_issues) = %+v, want the other two", got)
	}
	if got := allowedTools([]string{"read_issues", "get_score_summary", "get_search_console_data"}); len(got) != 0 {
		t.Fatalf("allowedTools(all disabled) = %+v, want none", got)
	}
	if got := allowedTools([]string{"unknown", "read_issues"}); len(got) != 2 {
		t.Fatalf("allowedTools(unknown+disabled) = %+v, want the other two", got)
	}
}
