...
func TestAllowedTools(t *testing.T) {
	...
	if len(all) != 8 {
		t.Fatalf("allowedTools(nil) has %d tools, want 8: %+v", len(all), all)
	}
	...
	for _, name := range []string{"read_issues", "get_score_summary", "get_search_console_data", "get_business_profile", "read_issue_work", "read_page", "render_chart", "update_business_profile"} {
		if !names[name] {
			t.Fatalf("allowedTools(nil) missing %s: %+v", name, all)
		}
	}
	if got := allowedTools([]string{"read_issues"}); len(got) != 7 {
		t.Fatalf("allowedTools(disabled read_issues) = %+v, want the other seven", got)
	}
	if got := allowedTools([]string{"read_issues", "get_score_summary", "get_search_console_data", "get_business_profile", "read_issue_work", "read_page", "render_chart", "update_business_profile"}); len(got) != 0 {
		t.Fatalf("allowedTools(all disabled) = %+v, want none", got)
	}
	if got := allowedTools([]string{"unknown", "read_issues"}); len(got) != 7 {
		t.Fatalf("allowedTools(unknown+disabled) = %+v, want the other seven", got)
	}
}
