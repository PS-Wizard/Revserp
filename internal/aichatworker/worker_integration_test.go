...
func TestDisabledToolsAreNotSentToProvider(t *testing.T) {
	...
	if len(provider.requests[0].Tools) != 5 {
		t.Fatalf("round tools = %+v, want the five non-disabled tools", provider.requests[0].Tools)
	}
	for _, def := range provider.requests[0].Tools {
...
func TestToolRoundCapSynthesizesFinalAnswer(t *testing.T) {
	...
	for i := 0; i < maxAgentRounds; i++ {
		if len(provider.requests[i].Tools) != 6 {
			t.Fatalf("round %d tools = %+v, want the six registered tools", i, provider.requests[i].Tools)
		}
	}
