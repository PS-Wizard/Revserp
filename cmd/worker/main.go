...
func run() error {
	...
	log.Printf("worker listening for queued crawls: manual_concurrency=%d manual_poll=%s auto_concurrency=%d auto_poll=%s auto_scheduler=%s crawl_page_worker_count=%d renderer_concurrency=%d obscura_path=%q obscura_timeout=%s obscura_kill_timeout=%s crawl_max_retries=%d crawl_retry_base=%s crawl_retry_max=%s", cfg.WorkerConcurrency, cfg.WorkerPollInterval, cfg.AutoCrawlConcurrency, cfg.AutoCrawlPollInterval, cfg.AutoCrawlSchedulerInterval, cfg.CrawlPageWorkerCount, cfg.RendererConcurrency, cfg.ObscuraPath, cfg.ObscuraTimeout, cfg.ObscuraKillTimeout, cfg.CrawlMaxRetries, cfg.CrawlRetryBase, cfg.CrawlRetryMax)
